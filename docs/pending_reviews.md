...
- [ ] 确认后端是否接收 `risk_components`、`llm_advisory` 和 `policy_chunk_ids`，并固化接口版本；
- [ ] 提供按时间可回放的数据版本号，支持模型滚动验证和回滚后的结果复现。
...
- [ ] 确认真实制度片段的生效日期、版本、可见角色及撤回机制；
- [ ] 确认 DeepSeek 候选建议可展示的字段，以及真实数据调用前的脱敏与审批要求。
